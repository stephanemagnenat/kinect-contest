...
const double max_diff_trans = 0.1;
const double min_R_rot = 0.5;
const double min_V_rot = 0.1;
...
int main(int argc, char** argv)
{
	...
	while (node.ok())
	{
		...
		if (abs(sqrt(alpha_i*alpha_i+beta_i*beta_i)-sqrt(alpha_o*alpha_o+beta_o*beta_o))>max_diff_trans)
		{
			ROS_WARN_STREAM("Translation difference too big: " <<
					abs(sqrt(alpha_i*alpha_i+beta_i*beta_i)-sqrt(alpha_o*alpha_o+beta_o*beta_o)));
			continue;
		}
